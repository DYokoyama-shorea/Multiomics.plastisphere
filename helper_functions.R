...
### modified nparcomp package to deal with more than 10000 data
my_own_nparcomp <-
  function (formula, data, type = c("Tukey", "Dunnett", "Sequen", 
                                    "Williams", "Changepoint", "AVE", "McDermott", "Marcus", 
                                    "UmbrellaWilliams", "UserDefined"), control = NULL, conf.level = 0.95, 
            alternative = c("two.sided", "less", "greater"), rounds = 3, 
            correlation = FALSE, asy.method = c("logit", "probit", "normal", 
                                                "mult.t"), plot.simci = FALSE, info = TRUE, contrast.matrix = NULL, 
            weight.matrix = FALSE) 
  {
    input.list <- list(formula = formula, data = data, type = type[1], 
                       conf.level = conf.level, alternative = alternative, 
                       asy.method = asy.method, plot.simci = plot.simci, control = control, 
                       info = info, rounds = rounds, contrast.matrix = contrast.matrix, 
                       correlation = correlation, weight.matrix = weight.matrix)
    ssq <- function(x) {
      sum(x * x)
    }
    logit <- function(p) {
          ...
          cl <- list(function() (t(dji[[x]]) %*% dji[[y]])/(n[j] * 
                     ...
                     function() -(t(dji[[x]]) %*% dij[[y]])/(n[v] * 
                                                               n[j] * n[i] * (n[i] - 1)), function() -(t(dij[[x]]) %*% 
                                                                                                         dji[[y]])/(n[i] * n[w] * n[j] * (n[j] - 
                                                                                                                                            1)))
          case <- (1:4)[p]
          rho.bf[x, y] <- rho.bf[y, x] <- sqrt(N * N)/sqrt(vd.bf[x] * 
                                                             vd.bf[y]) * cl[[case]]()
    ...
    if (type == "Dunnett" || type == "Tukey" || type == "Sequen") {
      for (t1 in 1:nc) {
        cmpid[t1] <- paste("p(", fl[which(Con[t1, ] == -1)], 
                           ",", fl[which(Con[t1, ] == 1)], ")")
      }
    }
    vii <- c(vi, vj)
    ...
    switch(asy.method, logit = {
      ...
      switch(alternative, two.sided = {
        ...
        for (pp in 1:nc) {
          p.adj[pp] <- 1 - pmvnorm(lower = -abs(T[pp]), 
                                   abs(T[pp]), corr = rho.bf, mean = rep(0, nc))
        }
        ...
        for (pp in 1:nc) {
          p.adj[pp] <- pmvnorm(lower = -Inf, T[pp], corr = rho.bf, 
                               mean = rep(0, nc))
        }
      ...
      probit.pd <- qnorm(pd)
      probit.dev <- diag(sqrt(2 * pi)/(exp(-0.5 * qnorm(pd) * 
                                             qnorm(pd))))
      ...
      df.sw <- round(df.sw)
      T <- sqrt(N) * (pd - 1/2)/sqrt(variances.bf)
      AsyMethod <- paste("Multi - T with", round(df.sw, rounds), 
                         "DF")
      switch(alternative, two.sided = {
        text.Output <- paste("True relative contrast effect p is not equal to 1/2")
        for (pp in 1:nc) {
          p.adj[pp] <- 1 - pmvt(lower = -abs(T[pp]), abs(T[pp]), 
                                corr = rho.bf, df = df.sw, delta = rep(0, 
                                                                       nc))
        }
        crit <- qmvt(conflevel, df = df.sw, corr = rho.bf, 
                     tail = "both")$quantile
        Lower <- pd - crit/sqrt(N) * sqrt(variances.bf)
        Upper <- pd + crit/sqrt(N) * sqrt(variances.bf)
      }, less = {
        text.Output <- paste("True relative contrast effect p is less than 1/2")
        for (pp in 1:nc) {
          p.adj[pp] <- pmvt(lower = -Inf, T[pp], corr = rho.bf, 
                            delta = rep(0, nc), df = df.sw)
        }
        crit <- qmvt(conflevel, corr = rho.bf, tail = "lower", 
                     df = df.sw)$quantile
        Lower <- rep(0, nc)
    ...
    if (plot.simci == TRUE) {
      ...
      Lowerp <- "|"
      plot(pd, 1:nc, xlim = c(0, 1), pch = 15, axes = FALSE, 
           xlab = "", ylab = "")
    ...
    Analysis <- data.frame(Comparison = cmpid, Estimator = round(pd, 
                                                                 rounds), Lower = round(Lower, rounds), Upper = round(Upper, 
                                                                                                                      rounds), Statistic = T, p.Value = p.adj)
    Overall <- data.frame(Quantile = crit, p.Value = min(p.adj))
    result <- list(Data.Info = data.info, Contrast = ch, Analysis = Analysis, 
                   Overall = Overall)
    if (info == TRUE) {
    ...
    if (weight.matrix == TRUE) {
      colW <- c()
      for (wc in 1:a1) {
        colW[wc] <- paste("p(", fl[vii[wc]], ",", fl[vjj[wc]], 
                          ")")
      }
      colnames(W) <- colW
...
### function to add letters in multiple comparison using nparcomp package 
func_letter_nparcomp <-
  function(x){
    library(tidyverse)
    library(rlang)
    input.data <- x$input$data 
    obj.tmp <- as.character(x$input$formula[2]) %>% rlang::data_sym()
    exp.tmp <- as.character(x$input$formula[3]) %>% rlang::data_sym()
    
    order <-
      input.data %>%
      group_by(!!exp.tmp) %>%
      summarize(mean = mean(!!obj.tmp, na.rm = TRUE)) %>%
      arrange(desc(mean)) %>%
      mutate(factor = fct_reorder(!!exp.tmp, mean)) %>%
      .$factor
    
    original.res <-
      x$Analysis %>%
      as_tibble() %>%
      mutate(Comparison = str_sub(Comparison, 4,-3)) %>%
      separate(Comparison, into = c("a", "b"), sep = " , ") %>%
      mutate(pair = map2(a,b,function(x,y){list(x,y) %>% factor(levels = order) %>% sort()})) %>%
      mutate(a = map_chr(pair, ~as.character(.[1])),
             b = map_chr(pair, ~as.character(.[2]))) %>%
    ...
    res <-
      crossing(as.character(order), as.character(order)) %>% 
      set_names("a", "b") %>%
      mutate(pair = map2(a,b,function(x,y){factor(list(x,y), levels = order) %>% sort()})) %>%
      nest(-pair) %>%
      dplyr::select(pair) %>%
      mutate(a = map_chr(pair, ~as.character(.[1])),
             b = map_chr(pair, ~as.character(.[2]))) %>%
      ...
      arrange(a,b) %>%
      dplyr::select(a,b) %>%
      left_join(.,original.res, by = c("a","b")) %>%
      mutate(sig = ifelse(p.Value < 0.05, "*", NA)) %>%
      nest(-a) %>%
      mutate(sig.num = map_int(data, function(x){x %>% filter(sig == "*") %>% nrow(.)}))%>%
      nest(-sig.num) %>%
      arrange(desc(sig.num)) %>%
      mutate(letter = letters[row_number()]) %>%
      unnest() %>%
      unnest() %>%
      filter(is.na(sig)) %>%
      nest(-b) %>%
      mutate(fin.letter = map_chr(data, ~.$letter %>% unique() %>% str_c(collapse = ""))) %>%
      dplyr::select(-data) %>%
      rename(param = "b") %>%
